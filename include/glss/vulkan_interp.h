...
// Owns a dedicated Vulkan compute pipeline for frame interpolation running
// embedded SPIR-V (shaders/interp.comp), with host-visible prev/curr/out buffers.
class VulkanInterpCompute {
public:
    explicit VulkanInterpCompute(VulkanContext& context);
    ~VulkanInterpCompute();

    VulkanInterpCompute(const VulkanInterpCompute&) = delete;
    VulkanInterpCompute& operator=(const VulkanInterpCompute&) = delete;
    VulkanInterpCompute(VulkanInterpCompute&&) = delete;
    VulkanInterpCompute& operator=(VulkanInterpCompute&&) = delete;

    bool Initialize(const uint32_t* spirv, size_t spirv_words);

    bool Dispatch(const FrameBuffer& prev, const FrameBuffer& curr, float t,
                  FrameBuffer& out, uint32_t mode = 0, int32_t search_radius = 4);
...
private:
    ...
    HostBuffer prev_buffer_;
    HostBuffer curr_buffer_;
