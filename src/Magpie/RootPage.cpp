...
void RootPage::InitializeComponent() {
	...
	// Set initial visibility for Home/About based on simple/advanced mode
...
void RootPage::NavigateToHomePage() {
	MUXC::NavigationView nv = RootNavigationView();
	// Home is the first menu item
	if (nv.MenuItems().Size() > 0) {
		nv.SelectedItem(nv.MenuItems().GetAt(0));
	}
}

void RootPage::RefreshNavigationVisibility() {
	// Hide Home and About items in simple mode
	try {
		bool isSimple = AppSettings::Get().IsSimpleMode();
		if (auto home = this->HomeNavigationViewItem()) {
			home.Visibility(isSimple ? Visibility::Collapsed : Visibility::Visible);
		}
		if (auto about = this->AboutNavigationViewItem()) {
			about.Visibility(isSimple ? Visibility::Collapsed : Visibility::Visible);
		}
	} catch (...) {
		// best-effort
	}
